...
function (_EventEmitterExtra) {
  ...
  _createClass(Stores, [{
    ...
    value: function update(name, nextState) {
      if (_lodash.default.isEqual(this.get(name), nextState)) return;

      _lodash.default.merge(this.state, _defineProperty({}, name, nextState));

      this.emit("update_".concat(name), this.state);
    }
  }]);
...
function useStore(to) {
  ...
  if (_lodash.default.isUndefined(stores.get(to))) stores.update(to, initial);
