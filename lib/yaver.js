...
function (_EventEmitterExtra) {
  ...
  _createClass(Stores, [{
    ...
    key: "update",
    value: function update(name, nextState) {
      if (!this.get(name)) this.storeBank[name] = {};
      if ((0, _isEqual.default)(this.get(name), nextState)) return;
      (0, _assign.default)(this.storeBank[name], nextState);
      this.emit("update_".concat(name), this.get(name));
    }
  }]);
...
function useStore(to) {
  var initial = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  /* Handle initial state */
  if ((0, _isUndefined.default)(stores.get(to))) stores.update(to, initial);
  ...
  var set = function set(nextState) {
    return stores.update(to, nextState);
  };
